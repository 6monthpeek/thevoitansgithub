...
async function assertSeniorOfficer(session) {
...
async function fetchGuildMemberSafe(userId) {
    const DISCORD_BOT_TOKEN = process.env.DISCORD_BOT_TOKEN;
    const DISCORD_GUILD_ID = process.env.DISCORD_GUILD_ID;
...
async function GET(req) {
    try {
        ...
        let logs = [];
        try {
            const buf = await fs_1.promises.readFile(LOG_FILE, "utf8");
            logs = JSON.parse(buf);
        ...
        filtered = filtered.filter((l) => {
            ...
            const byData = String(l?.data?.guildId || "").trim() === TARGET_GUILD ||
        ...
        if (userNeedle) {
            filtered = filtered.filter((l) => {
                const uid = String(l.userId ?? l?.data?.userId ?? l?.data?.author?.id ?? "").toLowerCase();
                const uname = String(l?.data?.resolvedUser?.username ?? l?.data?.userName ?? "").toLowerCase();
                const gname = String(l?.data?.resolvedUser?.globalName ?? l?.data?.globalName ?? "").toLowerCase();
                const nick = String(l?.data?.resolvedUser?.nickname ?? "").toLowerCase();
        ...
        const enriched = slice.map((l) => {
            ...
            const guildName = l?.data?.guildName ||
                l?.data?.guild?.name ||
                l?.data?.guildNameFallback ||
                l.guildId ||
                "Bilinmeyen Sunucu";
            // 4) Kullanıcı görünen ad (nickname > globalName > username > data.displayName)
            const userDisplayName = (u?.nickname && String(u.nickname).trim()) ||
                (u?.globalName && String(u.globalName).trim()) ||
                (u?.username && String(u.username).trim()) ||
                String(l?.data?.displayName || l?.data?.globalName || "").trim() ||
                "Bilinmeyen Kullanıcı";
            // 5) Username (yalın)
            const usernamePure = (u?.username && String(u.username).trim()) ||
            ...
            const avatarFromData = l?.data?.userAvatarUrl || l?.data?.memberAvatarUrl || l?.data?.userAvatar || undefined;
            const merged = {
                ...l,
                userId: fullId,
                userIdShort,
        ...
        return server_1.NextResponse.json({ ok: false, code: "UNHANDLED", error: e?.message || "failed" }, { status: 500 });
    }
}
