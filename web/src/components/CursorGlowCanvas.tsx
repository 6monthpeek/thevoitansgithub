...
type Props = {
  ...
  opacity?: number;            // global opaklık (0-1)
  fadeSpeed?: number;          // frame başına karartma yoğunluğu (0.01-0.1, düşük = daha yavaş kaybolma)
  baseRadius?: number;         // ana ışık yarıçapı (px)
  coreColor?: string;          // merkez parlak renk (rgba)
  midColor?: string;           // orta katman renk (rgba)
  outerColor?: string;         // dış halka renk (rgba)
  trailIntensity?: number;     // iz efekti yoğunluğu (0.5 - 1.5)
  followSharpness?: number;    // imleci takip hızı (0.05 - 0.3, yüksek = daha keskin takip)
  pointerLock?: boolean;       // false: sadece mousemove ile güncellenir
};

export default function CursorGlowCanvas({
  zIndex = 1,
  opacity = 0.85,
  fadeSpeed = 0.04, // Daha yavaş fade için daha düşük değer
  ...
  trailIntensity = 1.0,
  followSharpness = 0.12, // Orta-yüksek keskinlik
  pointerLock = false,
}: Props) {
  ...
  useEffect(() => {
    ...
    function drawFrame() {
      ...
      radialGradient(ctx, offset1X, offset1Y, extraRadius1, 
                     adjustColorAlpha(coreColor, 0.9), 
                     adjustColorAlpha(midColor, 0.7), 
                     adjustColorAlpha(outerColor, 0.5));
      ...
      radialGradient(ctx, offset2X, offset2Y, extraRadius2,
                     adjustColorAlpha(coreColor, 0.8),
                     adjustColorAlpha(midColor, 0.6),
                     adjustColorAlpha(outerColor, 0.4));
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden
      className="tv-cursor-glow pointer-events-none fixed inset-0"
...
function adjustColorAlpha(rgba: string, alpha: number): string {
  try {
