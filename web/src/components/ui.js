...
function NeonButton({ children, size = "md", variant = "primary", className, ...props }) {
    ...
    const base = "inline-flex items-center justify-center gap-2 rounded-full transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/40 " +
        sizes;
    if (variant === "outline") {
        return ((0, jsx_runtime_1.jsx)("button", { ...props, className: cn(base, "border border-white/10 text-zinc-200 hover:border-white/20 hover:bg-white/5 shadow-[inset_0_0_0_1px_rgba(255,255,255,.04)]", className), children: children }));
    }
    return ((0, jsx_runtime_1.jsx)("button", { ...props, className: cn(base, "font-semibold text-black bg-[color:var(--accent-cyan)] hover:bg-[#57dbff] shadow-[0_5px_14px_rgba(57,208,255,.15)]", className), children: children }));
}
/* Badge - small capsule */
